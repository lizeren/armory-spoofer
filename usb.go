...
func configureDevice(device *usb.Device) {
	...
	device.Descriptor.MaxPacketSize = 32
	...
	device.Descriptor.VendorId = 0x046d
	device.Descriptor.ProductId = 0xC539 // G903
	device.Descriptor.Device = 0x3906


	iManufacturer, _ := device.AddString(`Logitech`)
	device.Descriptor.Manufacturer = iManufacturer

	// iProduct, _ := device.AddString(`Storage Media`)
	// iProduct, _ := device.AddString(`mimik`)
	iProduct, _ := device.AddString(`USB Receiver`)
	...
	device.Descriptor.SerialNumber = iSerial - iSerial

	log.Printf("UniqueID: %s\r\n", fmt.Sprintf("%X", imx6ul.UniqueID()))
...
func buildMassStorageInterface() (iface *usb.InterfaceDescriptor) {
	...
	iface.InterfaceClass = 0x03 //Human Interface Device

	// iface.InterfaceSubClass = usb.SCSI_CLASS
	iface.InterfaceSubClass = 0x01 //Boot Interface Subclass

	// iface.InterfaceProtocol = usb.BULK_ONLY_TRANSPORT_PROTOCOL
	iface.InterfaceProtocol = 0x01 //Keyboard
